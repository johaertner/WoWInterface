
Perl_Config_Config = {
	["Anetheron-Adelgard"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Eckehard"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["MiniMapButtonPos"] = 300,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_ColorHealth"] = 0,
		["PositioningMode"] = 0,
		["Texture"] = 0,
	},
	["Nathrezim-Siegenot"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["MiniMapButtonPos"] = 300,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_ColorHealth"] = 0,
		["PositioningMode"] = 0,
		["Texture"] = 0,
	},
	["Blackrock-Kudgy"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_ColorHealth"] = 0,
		["TexturedBarBackround"] = 0,
		["Texture"] = 0,
	},
	["Nathrezim-Trautwin"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["MiniMapButtonPos"] = 300,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_ColorHealth"] = 0,
		["PositioningMode"] = 0,
		["Texture"] = 0,
	},
	["Nathrezim-Wenzelaus"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["MiniMapButtonPos"] = 300,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_ColorHealth"] = 0,
		["PositioningMode"] = 0,
		["Texture"] = 0,
	},
	["Kazzak-Kudgy"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Adelgunde"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["MiniMapButtonPos"] = 300,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_ColorHealth"] = 0,
		["PositioningMode"] = 0,
		["Texture"] = 0,
	},
	["Nathrezim-Wigbald"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
	["Area 52-Kudgy"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Warglgunde"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["MiniMapButtonPos"] = 300,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_ColorHealth"] = 0,
		["PositioningMode"] = 0,
		["Texture"] = 0,
	},
	["Nathrezim-Giselmund"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Guntram"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Rigobert"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
	["Azshara-Elliara"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Widukind"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
	["Ravencrest-Kudgy"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Friedebrecht"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Nevriye"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Anetheron-Bernhardates"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Die Silberne Hand-Arnun"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["TexturedBarBackround"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["PCUF_CastPartySupport"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonPos"] = 300,
		["ShowMiniMapButton"] = 1,
	},
	["Anetheron-Bernharda"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
	["Nathrezim-Erkenbald"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["ShowMiniMapButton"] = 1,
		["PCUF_InvertBarValues"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["PositioningMode"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_FadeBars"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_ColorHealth"] = 0,
		["TexturedBarBackround"] = 0,
		["Texture"] = 0,
	},
	["Nathrezim-Wendelbert"] = {
		["PCUF_ThreatIcon"] = 0,
		["PCUF_ColorFrameDebuff"] = 1,
		["PCUF_NameFrameClickCast"] = 0,
		["Texture"] = 0,
		["PCUF_InvertBarValues"] = 0,
		["PositioningMode"] = 0,
		["TexturedBarBackround"] = 0,
		["TransparentBackground"] = 0,
		["PCUF_ColorHealth"] = 0,
		["MiniMapButtonRad"] = 80,
		["MiniMapButtonPos"] = 300,
		["PCUF_FadeBars"] = 0,
		["PCUF_CastPartySupport"] = 0,
		["ShowMiniMapButton"] = 1,
	},
}
Perl_Config_Profiles = {
	"Nathrezim-Adelgunde", -- [1]
	"Nathrezim-Friedebrecht", -- [2]
	"Nathrezim-Eckehard", -- [3]
	"Nathrezim-Giselmund", -- [4]
	"Nathrezim-Rigobert", -- [5]
	"Nathrezim-Wenzelaus", -- [6]
	"Nathrezim-Nevriye", -- [7]
	"Nathrezim-Siegenot", -- [8]
	"Nathrezim-Widukind", -- [9]
	"Nathrezim-Erkenbald", -- [10]
	"Nathrezim-Wendelbert", -- [11]
	"Nathrezim-Guntram", -- [12]
	"Nathrezim-Trautwin", -- [13]
	"Nathrezim-Wigbald", -- [14]
	"Nathrezim-Warglgunde", -- [15]
	"Anetheron-Adelgard", -- [16]
	"Anetheron-Bernharda", -- [17]
	"Anetheron-Bernhardates", -- [18]
	"Ravencrest-Kudgy", -- [19]
	"Area 52-Kudgy", -- [20]
	"Die Silberne Hand-Arnun", -- [21]
	"Kazzak-Kudgy", -- [22]
	"Blackrock-Kudgy", -- [23]
	"Azshara-Elliara", -- [24]
}
Perl_Config_Global_ArcaneBar_Config = {
}
Perl_Config_Global_CombatDisplay_Config = {
}
Perl_Config_Global_Config_Config = {
}
Perl_Config_Global_Focus_Config = {
}
Perl_Config_Global_Party_Config = {
}
Perl_Config_Global_Party_Pet_Config = {
}
Perl_Config_Global_Party_Target_Config = {
}
Perl_Config_Global_Player_Config = {
}
Perl_Config_Global_Player_Buff_Config = {
}
Perl_Config_Global_Player_Pet_Config = {
}
Perl_Config_Global_Target_Config = {
}
Perl_Config_Global_Target_Target_Config = {
}
